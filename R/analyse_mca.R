...
fviz_mca_ind(res_mca, col.ind = "contrib",
...
Y <- res_mca0$l1[, 2]
stats <- cbind(Y, blocks) %>% pivot_longer(!Y)  %>% group_by(name) %>% anova_test(Y ~ value)
stats0 <- cbind(Y, blocks) %>% pivot_longer(!Y)  %>% group_by(name) %>% tukey_hsd(Y ~ value)
t_stats <- stats %>% as_tibble() %>% arrange(p) %>% filter(p<.05) %>% adjust_pvalue(method = "BH") %>% add_significance() %>% select(c("name", "p.adj", "p.adj.signif"))
t_stats0 <- stats0 %>% as_tibble() %>% arrange(p.adj) %>% filter(p.adj<.05)  %>% select(c("name", "group1", group2, "p.adj", "p.adj.signif"))
...
temp0 <- data.frame(cls = as.character(cl), blocks) %>% filter(!is.na(cls)) %>% select(-cls) %>% pivot_longer(everything()) %>% group_by(name) %>% nest()
