...
set_analysis(block_name = "clinic_tot_raw", to_remove = "26102")
colPers <- rev(colors_ind[3:2])
vars2 <- c("immun_aid_identifier", "weight", "height") # , "fatigue")
blocks <- blocks[[1]] %>% select(
    -c(
        vars2,
...
res_pca <- PCA(blocks, scale.unit = TRUE, ncp = 3, graph = FALSE)
```
...
ggboxplot(dat, x = "group", y = "Y", color = "group", palette = get_colors()[10:12]) +
...
## Numerical effect
```{r num_conf}
# age_at_inclusion_time BMI
i <- "BMI"
var <- clinic_intersect %>% pull(i)
dat <- data.frame(Y = (Y)) %>%
    mutate(var = (var + 1)^(1 / 2))
shapiro_test(dat$var)
cor(dat[, 1], dat[, 2], method = "spearman")
ggscatterstats(dat, Y, var, type = "nonparametric")
