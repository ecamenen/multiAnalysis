...
m2 <-  correlate(t_genes, cutoff = 0.7)
edge <- Edge(m3$r, m3$p)
...
n_genes <- as.matrix(node) %>% as.data.frame() %>% filter(as.numeric(size) < 40) %>% pull(id)
# n_genes <- as.matrix(node) %>% as.data.frame() %>% arrange(desc(size)) %>% head(7) %>% pull(id)
as.matrix(node) %>% as.data.frame() %>% column_to_rownames("id") %>% mutate(size = as.numeric(size))  %>% plot_bar(colour = c("#99000D", "gray"), n = 15)
