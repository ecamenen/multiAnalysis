...
#     values = list("protein_coding", seq(22)),
#     mart = db_annot
# )
# gene_query %>% filter(str_detect(hgnc_symbol, "SAA[12]"))
...
use_data(gene_query_entrez, overwrite = TRUE)
```
...
# i <- blocks[[1]]
# countData <- filtered_counts %>% select(rownames(i))
# dseq_ds <- DESeqDataSetFromMatrix(
#     countData = countData,
#     colData = i,
#     design = ~ batch + group
# )
# resultsNames(l_de0[[1]])
# 
# dds0 <- estimateSizeFactors(dseq_ds)
# dds1 <- estimateDispersions(dds0)
# dds <- nbinomWaldTest(dds1)
# ddss <- DESeq(dseq_ds)
# res <- results(ddss)
# counts(ddss, normalized=TRUE)
...
#   }
#   
#     k <- lapply(c(0, 1), function(j) filter(i, group == j)) %>%
#         .[c(rep(1, weigth_1), rep(2, weigth_2))]
#     i <- list.rbind(k)
#     countData <- filtered_counts[, sapply(k, rownames) %>% unlist()] %>%
#       set_colnames(rownames(i))
#     DESeqDataSetFromMatrix(
#         countData = countData,
#         colData = i,
#         design = ~ group
...
atlas_inflammation <- read.delim(file.path(path, "AIR_2023.txt")) %>%
  filter(SYMBOL != "") %>%
  select(-Description, -Name) %>%
  unique()
TERM2GENE <- select(atlas_inflammation, Map.name, SYMBOL)
...
plot_enrich(
    list(enrich_air[[1]][[1]], enrich_air[[2]][[1]]) %>% lapply(as.data.frame) %>% list.rbind(),
    type = "kegg",
    n = 15,
    title  = "Atlas of Inflammation\nResolution",
    wrap = 35
...
air_genes0 <-  list.map(
   c("Up-regulated", "Down-regulated"),
   f(x) ~ {
res <- as.data.frame(enrich_air[[x]]$`Control vs Disease`)
if(nrow(res) > 0)
    res %>% pull(geneID) %>% lapply(function(x) str_split(x, "/") %>% pluck(1)) %>% set_names(as.data.frame(enrich_air[[x]]$`Control vs Disease`) %>% pull(Description))
})
air_genes0 <- {
  res <- as.data.frame(enrich_air0$`Control vs Disease`)
      if(nrow(res) > 0)
  res %>% pull(geneID) %>% lapply(function(x) str_split(x, "/") %>% pluck(1)) %>% set_names(as.data.frame(enrich_air0$`Control vs Disease`) %>% pull(Description))
}

air_genes <- air_genes0 %>% unlist() %>% unique() %>% sort()
# as.data.frame(enrich_air0$`Control vs Disease`) %>% pull(geneID) %>% lapply(function(x) str_split(x, "/") %>% pluck(1)) %>% set_names(as.data.frame(enrich_air0$`Control vs Disease`) %>% pull(Description))
# air_genes0 %>%
#   list.flatten() %>%
#   set_names(names(.) %>% str_remove_all(".*\\.")) %>%
#   plot_venn(cex = 1.5)

immu_genes_univ <- pull(TERM2GENE, 2) %>% unique()
immu_genes0 <- intersect(unlist(de_genes1$`Control vs Disease`), immu_genes_univ) %>% unique() %>% sort()
# c(immu_genes0, top_genes1) %>% unique() %>% sort()
```
...
enrich_kegg0 <- list.map(
      list(node$id),
      f(i) ~ {
        res <- bitr(i, fromType = "ALIAS", toType = "ENTREZID", OrgDb = get(organism))
    if (length(res) > 0) {
        pull(res, 2) %>%
            head(250) %>%
            enrichKEGG(organism = "hsa")
...
list.map(
  ...
  f(a, b, c) ~ {
    list.map(
      ...
      f(x, y, z) ~ {
    #   tiff(
    #     file.path(path_img, "batch5", paste0("enrichr_", z, "_", c, ".tiff")),
    #     units = "px",
    #     width = 8000,
    #     height = 4000,
    #     res = 300
...
```{r}
i <- 1
name_immu <- c("(omplement)|(C2 )", "neutrophil", "(immun)|(inflammatory)", "(interleukins?)|(IL-?\\d)", "(interferon)|(IFN[ABG])", "mhc", "natural killer cell", "STAT[ 35]", "(macrophage)|(monocyte)", "(tumor necrosis factor)|(TNF)(NF-k)", "phago((cytosis)|(some))", "(Toll-like)|(TLR )", "leukocyte", " [BT] cell", "cytokine[^sis]", "RAGE", "(rostaglandin)|(arachidonic)|(icosa)", "(PUMA)|(TP53)|( p53)", "mTORC1")
dgea <- enriched0[[i]]
top_genes00 <- name_immu %>% lapply(function(x) get_genes_path0(dgea[-3], x)) %>% set_names(name_immu)
top_genes01 <- name_immu %>% lapply(function(x) get_genes_path0(list(enrich_kegg0[[i]]), x, name = "geneID", detect = "Description", p_adjust = "p.adjust", split_pattern = "/", path = "Description")) %>% set_names(name_immu)
top_genes0 <- top_genes00 %>% lapply(function(x) unlist(x) %>% unname() %>% unique() %>% sort()) %>% set_names(name_immu) %>% compact()
top_genes01 <- top_genes01 %>% lapply(function(x) unlist(x) %>% unname() %>% unique() %>% sort()) %>% set_names(name_immu) %>% compact()
top_genes1 <- unlist(top_genes0) %>% sort() %>% unname() %>% unique()
top_genes1 <- unlist(top_genes01) %>% bitr(fromType = "ENTREZID", toType = "SYMBOL", OrgDb = get(organism)) %>% pull(SYMBOL) %>% c(top_genes1) %>% sort() %>% unname() %>% unique()
top_genes <- c(immu_genes0, top_genes1) %>% unique() %>% sort()

list.map(
  c("Up-regulated", "Down-regulated"),
  f(x) ~ intersect(de_genes1$`Control vs Disease`[[x]], top_genes)
)

# setdiff(top_genes, unlist(de_genes1$`Control vs Disease`))
```
...
"(interleukins?)|(IL-?\\d)" %>%  get_genes_path0(dgea[-3], .) %>% pluck(1) %>% list.flatten() %>% set_names(names(.) %>% str_remove_all(".*\\.") %>% format_path(1000)) %>% plot_venn(n = 8, label = TRUE) + theme(legend.position = "none")
```
...
j <- names(p[[i]])[j]
tiff(
    file.path(path_img, "batch5", paste0("enrichr_", j, "_", i, ".tiff")),
    units = "px",
    width = 8000,
    height = 4000,
    res = 300
...
get_top0(l_de2[[1]], n = 15) %>%
    rownames() %>% 
    sort() %>%
    get_genes_path(enriched[[1]][[1]][-3], ., detect = "Genes", name = "Term", cutoff = 1)
enriched$`Up-regulated`$`Control vs Naive`[[1]] %>% as.data.frame() %>% filter(Adjusted.P.value <= 0.05) %>% arrange(desc(Combined.Score)) %>% head(15)
l_de$`Control vs Naive` %>% rownames_to_column() %>% filter(rowname  == "")
l_de2$`Control vs Naive` %>% get_top0(rank = TRUE) %>% filter(name  == "")
```

```{r, eval = FALSE} 
sort(ids) %>% sapply(get_ncbi_gene) %>% as.data.frame() %>% write_tsv("temp.tsv")
list.map(enriched[[1]][[1]], f(j) ~ filter(j, Adjusted.P.value <= 0.05) %>% select(c(1, 4, 9))) %>>% list.filter(.i != 3) %>% keep(function(x) nrow(x) > 0)

list.map(enriched[[1]][[1]], f(j) ~ filter(j, Adjusted.P.value <= 0.05) %>% pull(Term)) %>>% list.filter(.i != 3) %>% compact() %>% unlist() %>% unname() %>% sort() %>% as.data.frame() %>% write_tsv(file = file.path(cwd, "path.tsv"), col_names = FALSE)
...
lapply(ora, function(x) sapply(x, nrow))
...
paths <- list.map(dgea, f(j) ~ filter(j, Adjusted.P.value <= 0.05) %>% pull(Term) %>% format_path(100)) %>>% list.filter(.i != 3) %>% compact() %>% unlist() %>% unname() %>% sort() %>% str_remove_all("(\\(.*)|((ORPHA)|(WP)|(HSA)|(R-)).*") %>% str_trim() %>% to_title() %>% as.data.frame()

paths <- as.data.frame(enrich_kegg[[1]][[1]]) %>% filter(p.adjust <= 0.05) %>% pull(Description) %>% format_path(100) %>% sort() %>% to_title() %>% as.data.frame() %>% c(paths) %>% unlist() %>% sort() %>% as.data.frame()

write_tsv(paths, file = file.path(cwd, "sd_ctr_up.tsv"), col_names  = FALSE)
# name_immu0 <- c(name_immu, "cytokine(?!sis)" , "platelet")
...
name_immu0 <- c("(omplement)|(C2 )", "neutrophil", "(interleukins?)|(IL-?\\d)", "(interferon)|(IFN[ABG])", "natural killer cell", "STAT[ 35]", "(macrophage)|(monocyte)", "(tumor necrosis factor)|(TNF)(NF-k)", "phago((cytosis)|(some))", "(Toll-like)|(TLR )", "leukocyte", " [BT] cell", "cytokine[^sis]", "RAGE", "(rostaglandin)|(arachidonic)|(icosa)", "(PUMA)|(TP53)|( p53)", "mTORC1")
# TODO "monocytes?" granulocytes? MMP? NLRP? inflammasome? NK chemokine
# (rostaglandin)|(strogen)|(female sexual)|(Mammary gland)
# R[Hh][Oo]\\w? GTPase
MHC Class II Antigen
# ([Pp]latelet)|([Cc]oagulation)|(Hemostasis)|([Hh]eme)
# (defense)|(immune)|(inflamm)
top_genes <- cou[!str_detect(names(cou), "(neutrophil)|(fungus)|(regulation of immune)|(humoral)|(Cytokine)")]
c("neutrophil", "fungus", "regulation of")

# "(interleukins?)|(IL-?\\d)", "(interferon)|(IFN[ABG])", "NF-kB", "(rostaglandin)|(arachidonic)|(icosa)"
# "eutrophil [Dd]egranulation", "macrophage", "monocyte", "endritic", "granulocyte", "B cell", "mononuclear", "leukocyte aggregation", "leukocyte differentiation", "leukocyte chemotaxis"

  
c("eutrophil [Dd]egranulation", "macrophage", "monocyte", "endritic", "granulocyte", "B cell", "mononuclear", "leukocyte aggregation", "leukocyte differentiation", "leukocyte chemotaxis") %>% lapply(function(x) get_genes_path0(dgea[-3], x) %>% suppressMessages() %>% unlist() %>% unname() %>% unique() %>% sort()) %>% set_names("Neutrophil degranulation", "Macrophage activation", "Monocyte chemotaxis", "Dendritic cell differentiation", "Granulocyte chemotaxis", "B cell chemotaxis", "Mononuclear cell migration", paste("Leucocytes", c("aggregation", "differentiation", "chemotaxis"))) -> tmp
...
tmp0 <- list_table(temp)
(tmp3 <- list_common(temp))
lapply(names(tmp3), function(i) str_split(i, ":")) %>% unlist() %>% fct_count() %>% arrange(desc(n))

tmp0 %>%
    mutate(ID = rownames(.)) %>%
    gather("key", "value", -ID) %>%
    mutate(key = str_wrap(key, 20)) %>%
    mutate(ID = str_wrap(ID, 20)) %>%
    # mutate(key = factor(key, levels = order_resp)) %>%
    ggplot(aes(ID, key, fill = as.character(value))) + 
    geom_tile() +
    scale_fill_manual(
        values = c("grey", "#E41A1C"),
        na.value = "grey",
    ...
    theme(
        axis.title = element_blank(),
        axis.text.x = element_text(size = 10, color = "grey", angle = 90, hjust = 0.5, vjust = 0.5),
        axis.ticks = element_blank(),
        axis.line = element_blank(),
...
c(tmp[[1]], tmp[[2]]) %>% set_names(names(.) %>% str_remove_all("(Signaling)|(By)") %>% str_replace_all("Interleukin", "IL") %>% str_replace(" [aA]nd ", "\\/") %>% format_path(100)) %>% get_intersection()
...
list.map(c("(complement)|(C2 )", "(interferon)|(IFN[ABG])", paste0("((interleukins?)|(IL))-?", c(2, 10), "[ -/]")), get_genes_path0(c(enriched[[1]][[1]][-3], enriched[[2]][[1]][-3]),. , detect = "Genes", name = "Term"))

dgea$MSigDB_Hallmark_2020 %>% filter(Adjusted.P.value <= 0.05) %>% arrange(desc(Combined.Score)) %>% rownames_to_column() %>% filter(str_detect(Term, "(Interleukins?)|(IL-?\\d)"))
...
tmp <- enriched[[1]][[1]]$Reactome_2022 %>% filter(Adjusted.P.value <= 0.05) %>% pull(Term)

tmp <- enriched[[1]][[1]]$GO_Biological_Process_2021 %>% filter(Adjusted.P.value <= 0.05) %>% pull(Term)

tmp_n <- format_path(tmp, 100)


path <- str_replace_all(tmp, ".*(R-HSA-\\d*)", "\\1") %>% 
  term_similarity_from_Reactome() %>% 
  set_colnames(tmp_n) %>% set_rownames(tmp_n)
```
...
df0 <- simplifyEnrichment0(path)

path <- str_replace_all(tmp, ".*\\((GO:\\d*)\\)", "\\1") %>% 
  mgoSim(., ., semData = hsGO_BP, combine = NULL) %>% 
  set_colnames(tmp_n) %>% set_rownames(tmp_n)

path <- read_delim(file.path(cwd, "STRING_enrichmentProcess.csv")) %>% pull(term)
mat <- GO_similarity(path)
df <- simplifyGO(mat) %>% mutate(Term = tmp_n)

clus <- scale0(path) %>%
  get_dist0(method = hc_metric) %>% 
  hclust(method = hc_method)
# plotDendrogram(k = k, n = MAX_CLUSTERS, c = clus, cl = group)
group <- cutree(clus, k = k)
tiff(
    file.path(cwd, "RT.tiff"),
    units = "px",
    width = 10000 * cex,
    height = 1500 * cex,
    res = 600
...
simplifyEnrichment0 <- function (mat, method = "binary_cut", control = list(), plot = TRUE, verbose = TRUE, column_title = GetoptLong::qq("@{nrow(mat)} terms clustered by '@{method}'"), 
    ht_list = NULL, ...) 
{
    ...
    if (plot) 
        ht_clusters(mat, cl, column_title = column_title, 
            ht_list = ht_list, ...)
...
# gene2GO <- ViSEAGO::annotate(
#     "hsapiens_gene_ensembl",
#     Ensembl
# )
# use_data(gene2GO, overwrite = TRUE)
data("gene2GO")
...
BP <- create_topGOdata(
    geneSel = de_genes2[[1]][[1]],
    allGenes = rownames(counts_combatseq),
    gene2GO = gene2GO, 
    ont = "BP",
    nodeSize = 5
)
...
bp_res <- merge_enrich_terms(Input = list(condition = c("BP", "classic")))
...
blocks2 <- list.map(
  ...
  f(i) ~ {
    list.map(
      ...
      arrange(rowname) %>% 
      column_to_rownames() #%>%
      # select(rownames(blocks11[[i]]))
  }
)
j <- 1
i <- names(l_de0)[j]
(colnames(blocks2[[i]]) == rownames(blocks11[[i]])) %>% all()
...
row_annotation <- data.frame(Group = y)
names_levels <- unique(unlist(y))
# row_annotation[1, ] <- "GC-Corticoid + aIL1"
# row_annotation[2, ] <- "aIL1 only"
res_scaled <- blocks <- t(blocks2[[i]]) # %>% as.data.frame() %>% update_columns(colnames(.), function(x) log2(x + 1e-10))
...
```{r, eval = FALSE}
ego <- enrichGO(
  # gene = pull(de_genes2[[1]][[1]], ensembl_ids),
  de_genes2[[1]][[1]],
  ont = "ALL",
  keyType = "ENSEMBL",
...
entrez_genes <- list.map(de_genes_ranked, f(i) ~ {
...
sapply(entrez_genes, length)

univ <- ensembl_ids %>% bitr(fromType = "ENSEMBL", toType = "ENTREZID", OrgDb = get(organism)) %>% pull("ENTREZID")
kegg_genes1 <- list.map(
  de_genes2,
  f(i) ~ {
    list.map(
      i,
      ~ bitr(., fromType = "ENSEMBL", toType = "ENTREZID", OrgDb = get(organism)) %>% 
        pull("ENTREZID")
    )
    
}
)
lapply(kegg_genes1, function(x) sapply(x, length))

data("gene_query_entrez")
TERM2GENE2 <- inner_join(rename(TERM2GENE, hgnc_symbol = SYMBOL), gene_query_entrez) %>% select(-hgnc_symbol)

gsea_air <- GSEA(entrez_genes[[1]], TERM2GENE = TERM2GENE2) %>%
...
list.map(
    ...
    f(i, j, k) ~ {
        ...
        heatplot0(i, foldChange = entrez_genes[[k]]) + 
        labs(title = k) + 
        theme(legend.position = "none")
    }
)  %>% plot_grid(
...
library("pathview")
pathview(gene.data = entrez_genes[[1]], species="hsa", pathway.id="hsa04060", low = list(gene = "#377EB8", cpd = "blue"), mid = list(gene = "gray", cpd = "gray"), high = list(gene = "#E41A1C", cpd =
"yellow"))
```
...
library("meshes")
gse_mesh <- lapply(entrez_genes, function(x) gseMeSH(x, MeSHDb = db_mesh, database = 'gene2pubmed', category = "G"))
```

```{r, eval = FALSE}
library("DOSE")
gse_do <- lapply(entrez_genes, function(x) gseDO(x, verbose = FALSE))
gse_dgn <- lapply(entrez_genes, function(x) gseDGN(x, verbose = FALSE))
...
# Wiki Pathway
ego2 <- enrichWP(
    gene = kegg_genes1[[1]][[1]],
    universe = univ,
    organism = "Homo sapiens",
    qvalueCutoff = 0.05
)
# pairwise_termsim(ego2) %>% treeplot(showCategory = 200)
...
pairwise_termsim(ego3) %>% treeplot(showCategory = 200)
cnetplot(ego3, color.params = list(foldChange=entrez_genes[[1]], category  = "grey"), cex.params = list(category_label = 1.5), showCategory = 1, node_label = "gene") + scale_color_gradientn(name="Log2(FC)", colours  = brewer.pal(9, "Reds") ) + guides(size = "none")
...
db_msig <- msigdbr(species = "Homo sapiens", category = "H")
...
# db_msig2 <- msigdbr(species = "Homo sapiens", category = "C2")
...
# msigdbr(species = "Homo sapiens", category = "C2") %>% pull(gs_description) %>% unique() %>% discard(function(x) str_detect(x, "(^Genes )|(Up\\-)|(Down\\-)|(Cluster)|(Amplification)"))
```

# Plot GSEA
```{r, eval = FALSE}
# gse <- list(gse_go, gsea_kegg, gse_wiki, gse_do, gse_dgn, gse_mesh, gse_msig)#, gse_msig2)
gse <- list(gse_go, gsea_kegg, gse_wiki, gse_mesh, gse_msig)
# use_data(gse, overwrite = TRUE)
# titles <- c("GO", "KEGG", "WikiPathway", "Disease Ontology", "Disease Gene Network", "MeSH", "MSigDb")
titles <- c("GO", "KEGG", "WikiPathway", "MeSH", "MSigDb (H)")
paths2 <- lapply(gse, function(x) x[[1]] %>% as.data.frame() %>% filter(p.adjust <= 0.05) %>% pull(Description)) %>% unlist() %>% sort()
lapply(name_immu, function(x) paths2 %>% .[str_detect(., x)]) %>% set_names(name_immu) %>% compact()
p3 <- lapply(
  names(l_de),
  function (x) {
    print(x)
