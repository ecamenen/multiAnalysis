...
batches <- pull(batch_df, 2) %>% set_names(pull(batch_df, 1) %>% str_remove_all("A") %>% as.numeric())

id_ctrl <- filter(clinic_transf, str_detect(disease, "control")) %>% pull(immun_aid_identifier)
...
counts_matrix <- lapply(l_visits, function(i) select(i, matches(paste0("^", common_rows, "$", collapse = "|")))) %>% list.cbind() %>% select(-contains("GeneName"))
...
top_genes0 <- c(up_genes, down_genes) %>% unlist() %>% sort() %>% unique() %>% list() %>% set_names(names(blocks))
