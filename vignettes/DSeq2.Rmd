...
set_analysis(block_name = "clinic", type = "qual", cwd = cwd)
...
#     function(i) {
#         read_excel(
#             path = file.path(golem::get_golem_wd(), "inst", "extdata", "Updated List of patients- Batch1,2,3, 4 and 5 June 2023.xlsx"),
#             skip = 1,
#             sheet = i
#         ) %>% select(contains(c("Patient", "Disease")))
#     }
# ) %>% list.rbind() %>%
#   mutate(`Patient ID` = as.numeric(`Patient ID`)) %>%
#   distinct() %>%
#   filter(!(`Patient ID` == "6019" & str_detect(Disease, "unknown"))) %>%
#   set_rownames(pull(., 1))

batch_df <- read_tsv(file.path(cwd, "inst", "extdata", "batch_rna.tsv")) %>% filter(str_detect(Sample, "A"))
batches <- pull(batch_df, 2) %>% set_names(pull(batch_df, 1) %>% str_remove_all("A05PB\\d") %>% as.numeric())
batches["6052"] <- 4
...
counts_matrix <- read_tsv(file = file.path(path, "gene_counts_all_batch4.tsv"))
```
...
blocks <- lapply(
    c("control", "osteitis"),
    function(x) filter(blocks, str_detect(C_2643_3798, x)) %>% rownames()
) %>% 
...
```{r, eval = FALSE}
GCC_V2 <- c(3005, 9002, 25008, 26102)[-1]
# batches[names(batches) %in% gccs]
aIL1_V2 <- c(13005, 13010, 13016)
control <- filter(clinic_transf, str_detect(disease, "control")) %>% pull(immun_aid_identifier)
naive <- filter(blocks, Naive == 1) %>%
    rownames() %>%
    as.numeric()
GCC_V1 <- filter(blocks, Corticosteroid == 1 & Anti.IL1 != 1 & Anti.IL6 != 1) %>%
    rownames()
aIL1_V1 <- filter(blocks, Corticosteroid != 1 & Anti.IL1 == 1 & Anti.IL6 != 1) %>%
    rownames()
...
blocks[2:3] <- list.map(
  ...
  list.map(
    c("A", "B"),
    f(i, j) ~ {
    data.frame(id = get(paste0(x, "_V", j)), group = factor(j - 1)) %>% mutate(batch = batches[names(batches) %in% id] %>% factor()) %>% column_to_rownames(var = "id")
  }) %>% list.rbind()
})

# %>% set_names(paste0(c("GCC", "aIL1")," V1 vs V2"))

blocks[4:5] <- list.map(list(GCC_V2, aIL1_V2), f(i, j, k) ~ data.frame(id = rep(i, 2), group = rep(c(0, 1), each = 3) %>% factor()) %>% mutate(batch = rep(batches[names(batches) %in% id], 2) %>% factor(), id = paste0(rep(c("A", "B"), each = 3), ".", id)) %>% column_to_rownames(var = "id"))

blocks[[5]]$batch <- rep(1, 6) %>% factor()

blocks <- set_names(blocks, "Control vs Naive", c("GCs", "aIL1") %>% paste(., "Non-responder vs", ., "Responder"), c("GCs", "aIL1") %>% paste(., "Naive vs", ., "Responder"))

# list.map(list(GCC_v2, aIL1_v2), f(x) ~ list.map(l_visits, f(i, j) ~ select(i, as.character(x)) #%>% set_colnames(paste0(LETTERS[j], colnames(.)))
#                                                ) %>%
#     list.cbind() %>%
...
#     c("BCL6", "IL10", "VEGFA")
# )
top_genes <- list.map(l_de2, get_top0(., n = 15))
list.map(l_de2, f(i, j, k) ~ volcano_plot(i, top_genes = top_genes[[j]], cex = 1, legend = "none", title = "") + xlab("") + ylab("")) -> p2
# for (pl in names(p2)) {
...
# Significant genes
```{r}
de_genes0 <- list.map(l_de2, f(i) ~ get_top0(i, n = 1000) %>%
    rownames() %>%
    str_remove_all("_\\d") %>%
    unique())
...
top_genes <- c(up_genes, down_genes) %>% unlist() %>% sort() %>% unique() %>% list() %>% set_names(names(blocks))
```
...
```{r, eval = FALSE}
genes_path <- get_genes_path(enriched[[1]][[1]], c("Complement Activation", "Interferon Alpha Response", "Interferon Gamma Response", "Classical Antibody-Mediated Complement Activation"))
# BATF2 C1QA C1QB C1QC CD38 CFB CMPK2 EPSTI1 FCGR1A IFI27 IFI35 IFI44 IFI44L IFIT3 IFITM3 ISG15 LAP3 METTL7B MTHFD2 MX1 OASL PARP9 PIM1 RSAD2 SERPING1 SOCS3 TNFAIP6 USP18 VAMP5
genes_path <- get_genes_path(enriched[[2]][[1]], query)
genes_path <- get_genes_path(enriched[[1]][[3]], "regulation of lymphocyte migration")
# CCR6 GCSAM
genes_path <- get_genes_path(enriched[[2]][[4]], c("complement activation", "Creation Of C4 And C2", "IL-2/STAT5 Signaling", "Interferon Gamma Response", "TNF-alpha Signaling"))
# BCL3 C1QA C1QC CDC6 EGR1 FCGR1A FURIN HIF1A ICAM1 IGLL5 IL4R LDLR PFKFB3 PIM1 RGS16 SLC2A3 SOD2 TNIP1 UPP1 
get_intersection0(genes_path)
plot_venn1(genes_path, ratio = 0.3, n = 6)
genes_path <- get_genes_path(enriched[[2]][[2]], c("negative regulation of receptor signaling pathway", "neutrophil", "Interferon Gamma Response", "TNF-alpha Signaling via NF-kB"))
# ATP6V0C BCL3 CD63 FCGR1A LGALS3 PFKFB3 PIM1 RNASE2 SLC2A3 SOCS3 TRIB1 TUBB2A TUBB4B
genes_path <- get_genes_path(enriched[[2]][[5]], c("leukocyte aggregation", "B cell chemotaxis", "Complement Activation ", "Type II interferon signaling ", "IL-6/JAK/STAT3 Signaling", "Inflammatory Response", "Interferon Alpha Response", "Interferon Gamma Response"))
get_intersection(genes_path[-c(seq(3))])
get_intersection(genes_path[-c(6:8)])
get_intersection(genes_path[-c(4:5, 7)])
# ADM AQP9 BATF2 C1QA C1QB C1QC C2 CCR1 CFB FCGR1A FFAR2 FPR1 GAS6 GBP1 GNA15 HSD3B7 ICAM1 IDO1 IFI27 IFITM3 IFNGR2 IL1B LAP3 MARCO METTL7B MT2A NAMPT PIM1 S100A8 S100A9 SCARF1 SLC31A2 SOCS3 STAB1 TNFAIP6 VAMP5
genes_path <- get_genes_path(enriched[[2]][[3]], c("negative regulation of regulatory T cell", "response to type I interferon", "Type II interferon signaling", "STAT5 Activation Downstream", "IL-2/STAT5 Signaling", "IL-6/JAK/STAT3 Signaling", "Inflammatory Response", "Interferon Alpha Response", "Interferon Gamma Response", "TNF-alpha Signaling"))
# ADM BCL2L1 BCL3 CCNE1 CDKN1A CKAP4 CMPK2 CTLA4 EPSTI1 FFAR2 FOSL1 GADD45B GMPR ICAM1 IFI44 IRF1 ISG15 JUN LAP3 LDLR LGALS3BP MUC1 MX1 MXD1 NFKBIA RGS1 SCARF1 SIK1 SLC2A3 SOCS3 TGM2 TNFRSF12A ZC3H12A
get_intersection(genes_path[-c(seq(5))])
get_intersection(genes_path[c(seq(5))])
get_intersection(genes_path[c(seq(3), 9:10)])
query <- list.map(p[[2]][[1]], filter(.$data, Adjusted.P.value <= 0.05) %>% pull(Term)) %>% unlist() %>% unique() %>% sort() %>% str_remove_all("((ORPHA)|(WP)|(HSA)|(R-)).*") %>% str_remove_all(" \\(.*\\)") %>% str_trunc(30)

list.map(p[[2]][[5]], filter(.$data, Adjusted.P.value <= 0.05) %>% pull(Term)) %>% unlist() %>% unique() %>% sort() %>% data.frame(id = .) %>% write_tsv("temp.tsv", col_names = FALSE)

top_genes <- list(
`Control vs Naive (Up)` = c("BATF2", "C1QA", "C1QB", "C1QC", "CD38", "CFB", "CMPK2", "EPSTI1", "FCGR1A", "IFI27", "IFI35", "IFI44", "IFI44L", "IFIT3", "IFITM3", "ISG15", "LAP3", "METTL7B", "MTHFD2", "MX1", "OASL", "PARP9", "PIM1", "RSAD2", "SERPING1", "SOCS3", "TNFAIP6", "USP18", "VAMP5"),

`aIL1 Naive vs aIL1 Responder (Up)` = c("CCR6", "GCSAM"),

`GCs Naive vs GCs Responder (Up)` = c("CD4", "CD244"),


`GCs Non-responder vs GCs Responder (Down)` = c("ATP6V0C", "BCL3", "CD63", "FCGR1A", "LGALS3", "PFKFB3", "PIM1", "RNASE2", "SLC2A3", "SOCS3", "TRIB1", "TUBB2A", "TUBB4B"),

`aIL1 Non-responder vs aIL1 Responder (Down)` = c("ADM", "BCL2L1", "BCL3", "CCNE1", "CDKN1A", "CKAP4", "CMPK2", "CTLA4", "EPSTI1", "FFAR2", "FOSL1", "GADD45B", "GMPR", "ICAM1", "IFI44", "IRF1", "ISG15", "JUN", "LAP3", "LDLR", "LGALS3BP", "MUC1", "MX1", "MXD1", "NFKBIA", "RGS1", "SCARF1", "SIK1", "SLC2A3", "SOCS3", "TGM2", "TNFRSF12A", "ZC3H12A"),

`GCs Naive vs GCs Responder (Down)` = c("BCL3", "C1QA", "C1QC", "CDC6", "EGR1", "FCGR1A", "FURIN", "HIF1A", "ICAM1", "IGLL5", "IL4R", "LDLR", "PFKFB3", "PIM1", "RGS16", "SLC2A3", "SOD2", "TNIP1", "UPP1"),

`aIL1 Naive vs aIL1 Responder (Down)` = c("ADM", "AQP9", "BATF2", "C1QA", "C1QB", "C1QC", "C2", "CCR1", "CFB", "FCGR1A", "FFAR2", "FPR1", "GAS6", "GBP1", "GNA15", "HSD3B7", "ICAM1", "IDO1", "IFI27", "IFITM3", "IFNGR2", "IL1B", "LAP3", "MARCO", "METTL7B", "MT2A", "NAMPT", "PIM1", "S100A8", "S100A9", "SCARF1", "SLC31A2", "SOCS3", "STAB1", "TNFAIP6", "VAMP5")
)

top_genes0 <- top_genes
top_genes0[[2]] <- c(top_genes[[2]], top_genes[[5]])
top_genes0[[3]] <- c(top_genes[[3]], top_genes[[6]])
top_genes0 <- top_genes0[c(1:4, 7)]
top_genes0 <- compact(top_genes0) %>% set_names(names(.) %>% str_remove_all(" \\(.*\\)"))
names(top_genes0)[2] <- "aIL1 Non-responder vs aIL1 Responder"

plot_venn1(top_genes[-c(2:3)], ratio = .4, wrap = 20)
get_intersection0(top_genes[-c(2:3)])
get_intersection(top_genes[-c(2:3)], n = 2) %>% unlist() %>% unname() %>% data.frame(id = .) %>% save_tsv("temp.tsv", col_names = FALSE)
```
...
list.map(
    ...
    f(i, j, k) ~ {
        # setReadable(i, OrgDb = get(organism), keyType = "ENTREZID") %>%
            heatplot0(foldChange = kegg_genes[[k]]) + 
            labs(title = k) + theme(legend.position = "none")
    }
)  %>% plot_grid(
    plotlist = .,
...
list.map(
  ...
  f(i, j, k) ~ {
    ...
    #     res = 300
    # )
    plot_enrich(x, wrap = 25, n = 11, type = "kegg", title = "", paste0(z, " (", str_to_title(k), ")") %>% str_wrap(20))
        })
    # dev.off()
}) -> p
...
```
```{r}
# gse_wiki <- gseWP(kegg_genes[[1]], organism = "Homo sapiens") %>%
#   setReadable(OrgDb = get(organism), keyType = "ENTREZID")
...
library(DOSE)
gse_do <- gseDO(kegg_genes[[1]], verbose = FALSE)
gse_dgn <- gseDGN(kegg_genes[[1]], verbose = FALSE)
...
library(meshes)
gse_mesh <- gseMeSH(kegg_genes[[1]], MeSHDb = db, database = 'gene2pubmed', category = "G")
```
```{r}
p$GO <- plot_enrich(gse_go_cro[[1]] , n = 25, wrap = 50, type = "gsea", title = "GO")
p$KEGG <- plot_enrich(gsea_kegg[[1]] , n = 25, wrap = 50, type = "gsea", title = "KEGG")
p$WIKI <- plot_enrich(gse_wiki , n = 25, wrap = 50, type = "gsea", title = "WikiPathway")
# p$DO <- plot_enrich(gse_do , n = 25, wrap = 50, type = "gsea", title = "Disease Ontology")
p$DGN <- plot_enrich(gse_dgn , n = 25, wrap = 50, type = "gsea", title = "Disease Gene Network")
p$MESH <- plot_enrich(gse_mesh , n = 25, wrap = 50, type = "gsea", title = "MeSH")
p$MSIG <- plot_enrich(gse_msig2 , n = 25, wrap = 50, type = "gsea", title = "MSigDb")
...
```{r, eval = FALSE}
library(msigdbr)
db_msig <- msigdbr(species = "Homo sapiens")
msig_gene <- select(db_msig, gs_name, entrez_gene)
msig_description <- select(db_msig, gs_name, gs_description)
gse_msig <- GSEA(kegg_genes[[1]], TERM2GENE = msig_gene, TERM2NAME = msig_description) %>%
  setReadable(OrgDb = get(organism), keyType = "ENTREZID")

db_msig2 <- msigdbr(species = "Homo sapiens", category = "C7")
msig_gene2 <- select(db_msig2, gs_name, entrez_gene)
msig_description2 <- select(db_msig2, gs_name, gs_description)
gse_msig2 <- GSEA(kegg_genes[[1]], TERM2GENE = msig_gene2, TERM2NAME = msig_description2) %>%
  setReadable(OrgDb = get(organism), keyType = "ENTREZID")
```
