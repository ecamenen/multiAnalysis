...
responses <- gather(responses0, "treatment", "response") %>%
...
batch_df <- read_tsv(file.path(path, "batch_rna.tsv")) 
...
GCC_V2 <- filter(blocks[[2]], Corticosteroid == 1 & `Anti-IL1` != 1 & `Anti-IL6` != 1 & `NSAIDs` != 1 & `DMARDs` != 1 & `Other` != 1  & `Anti-TNF` != 1) %>%
    rownames()
GCC_aIL1_V2 <- filter(blocks[[2]], Corticosteroid == 1 & `Anti-IL1` == 1 & `Anti-IL6` != 1 & `NSAIDs` != 1 & `DMARDs` != 1 & `Other` != 1  & `Anti-TNF` != 1) %>%
    rownames()
GCC_aIL1_V1 <- filter(blocks[[1]], Corticosteroid == 1 & `Anti-IL1` == 1 & `Anti-IL6` != 1 & `NSAIDs` != 1 & `DMARDs` != 1 & `Other` != 1  & `Anti-TNF` != 1) %>%
    rownames()
aIL1_V2 <- filter(blocks[[2]], Corticosteroid != 1 & `Anti-IL1` == 1 & `Anti-IL6` != 1) %>%
    rownames()
naive <- filter(blocks[[1]], Naive == 1) %>%
    rownames()
GCC_V1 <- filter(blocks[[1]], Corticosteroid == 1 & `Anti-IL1` != 1 & `Anti-IL6` != 1 & `NSAIDs` != 1 & `DMARDs` != 1 & `Other` != 1 & `Anti-TNF` != 1) %>%
    rownames()
aIL1_V1 <- filter(blocks[[1]], Corticosteroid != 1 & `Anti-IL1` == 1 & `Anti-IL6` != 1 & `NSAIDs` != 1 & `DMARDs` != 1 & `Other` != 1 & `Anti-TNF` != 1) %>%
    rownames()
...
blocks[2:4] <- list.map(
  ...
  list.map(
    c("A", "B"),
    f(i, j) ~ {
    data.frame(id = get(paste0(x, "_V", j)), group = factor(j - 1)) %>% 
        mutate(batch = batches[names(batches) %in% id] %>% factor()) %>% 
...
blocks[5:7] <- list.map(
  list(GCC_V2, aIL1_V2, GCC_aIL1_V2), 
  f(i, j, k) ~ {
    data.frame(id = rep(i, 2), group = rep(c(0, 1), each = length(i)) %>% factor()) %>% 
      mutate(
        batch = rep(batches[names(batches) %in% id], 2) %>% factor(), 
        id = paste0(rep(c("A", "B"), each = length(i)), ".", id)
        ) %>% 
      column_to_rownames(var = "id")
    })

blocks[8:10] <- list.map(
  c("GCC", "aIL1", "GCC_aIL1"), 
  f(x) ~ {
  list.map(list(naive, get(paste0(x, "_V1"))), f(i, k) ~ data.frame(id = i, group = factor(k - 1))) %>%
    list.rbind() %>%
    arrange(id) %>%
    set_rownames(pull(., id) %>% paste0("A.", .)) %>%
    mutate(batch = batches[names(batches) %in% id] %>% factor()) %>%
...
blocks <- set_names(
  ...
  c(
    "Control vs Naive", 
    c("GCs", "aIL1", "GCC+aIL1") %>% paste(., "Non-responder vs", ., "Responder"), 
    c("GCs", "aIL1", "GCC+aIL1") %>% paste(., "Naive vs", ., "Responder"),
    c("GCs", "aIL1", "GCC+aIL1") %>% paste("Naive vs", ., "Non-responder")
  )
)
```
