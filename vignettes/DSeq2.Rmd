...
de_genes <- list.map(l_de, f(i) ~ mutate(i, log2fc = log2FoldChange) %>%
    filter(!is.na(padj)) %>%
    get_top(0.5, 0.05, n = 200) %>%
    rownames() %>%
    str_remove_all("_\\d") %>%
    unique())
cbind(apply(blocks, 2, sum) %>% as.data.frame() %>% set_colnames(c("N. patients")), sapply(de_genes, length) %>% as.data.frame() %>% set_colnames(c("N. significant genes"))) %>% kable0()
