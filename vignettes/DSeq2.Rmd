...
```{r}
de_genes <- list.map(l_de, f(i) ~ mutate(i, log2fc = log2FoldChange) %>% filter(!is.na(padj)) %>% get_top(0.5, 0.2, n = 200) %>% filter(!str_detect(rownames(.), "Unknown")) %>% rownames() %>%
    str_remove_all("_\\d") %>%
    unique())
...
p <- list.map(gse, f(i) ~ data.frame(i)) %>%
    ...
    list.map(f(i, j, k) ~ data.frame(i) %>% mutate(Adjusted.P.value = p.adjust, Term = str_to_title(Description), Count = setSize, Overlap = core_enrichment) %>% plot_enrich(10, gsea = TRUE, title = k)) %>%
    # list(pluck(., 1), pluck(., 2), NULL, pluck(., 3), pluck(., 4)) %>%
    plot_grid(
        plotlist = .,
        nrow = 2,
