...
set_analysis(block_name = "emna_treat", type = "qual", cwd = cwd)
...
batch_df <- read_tsv(file.path(cwd, "inst", "extdata", "batch_rna.tsv")) 
...
counts_matrix <- counts_combatseq
```
...
common_rows <- Reduce(intersect, list(colnames(counts_matrix), c(rownames(blocks), control, rownames(treat_v2)), names(batches)))
counts_matrix <- lapply(l_visits, function(i) select(i, matches(paste0("^", common_rows, "$", collapse = "|")))) %>% list.cbind() %>% select(-contains("gene_id"))

blocks <- blocks %>% mutate(ID = rownames(.)) %>% filter(ID %in% common_rows) %>% select(-ID) %>% select(contains(c("Mucocutaneous", "Musculoskeletal", "Urticaria", "Myocarditis", "Anti-IL1", "Anti-IL6", "Corticosteroid", "DMARDs", "Kinase.Inhibitor", "Naive")))
...
blocks <- lapply(
    ...
    c("control", "osteitis"),
    function(x) filter(blocks, str_detect(C_2643_3798, x)) %>% pull(C_2643_3796) %>% as.numeric
) %>% 
...
```{r}
GCC_V2 <- filter(treat_v2, Corticosteroid == 1 & `Anti-IL1` != 1 & Methotrexate != 1) %>% rownames()
# batches[names(batches) %in% gccs]
aIL1_V2 <- filter(treat_v2, `Anti-IL1` == 1 & Corticosteroid != 1 & Methotrexate != 1) %>% rownames()
...
name_immu <- c("(complement)|(C2 )", "neutrophil", "(immune)|(inflammatory)", "(interleukins?)|(IL-?\\d)", "(interferon)|(IFN[ABG])", "mhc", "natural killer cell", "STAT[ 35]", "macrophage", "(tumor necrosis factor)|(TNF)(NF-k)", "phago((cytosis)|(some))", "(Toll-like)|(TLR )", "WNT", "leukocyte", " [BT] cell", "(TNF-alpha)|(NF-k(appa)?B)")
top_genes1 <- name_immu %>% lapply(function(x) get_genes_path0(dgea[-3], x) %>% suppressMessages() %>% unlist() %>% unname() %>% unique() %>% sort()) %>% set_names(name_immu) %>% compact()
...
l_cyt <- list.map(blocks11, f(x) ~ rownames(x) %>% c("geneName", .) %>% select(cyt, .))
...
vars <- c("IL1A", "IL1B", "IL4", "IL4", "IL12", "IL13", "IFN1", "IFNG", "IFNL")
list.map(
  # pull(cyt, "geneName"),
  vars,
  f(j, jj) ~ {
    list.map(tmp, f(k) ~ select(k, j)) %>% 
      ...
      plot_violin0(
        ...
        colour = c(colour_var[jj], alpha(colour_var[jj], 0.5)),
        color_title = colour_var[jj],
        wrap = 5,
        size = 3,
        pch_colour = "gray50",
        wrap_title = 20,
