...
set_analysis(block_name = "rna_clinic_still", type = "qual")
rna <- blocks[[2]]
...
treat <- attributes(clinic_intersect)$codes %>%
    ...
    pull(item_name)
vars <- c("NSAIDs", "Paracetamol", "Prednisolone", "Biologic")
filename_top_genes <- file.path(path_out, "top_genes_muscu.temp.tsv")
```
...
## Impacted organ
df <- cbind(rna, muc = clinic_intersect$musculoskeletal) %>%
    tibble() %>%
    mutate(cl = factor(muc)) %>%
    select(-c(muc))

# res <- select(exp,-seq(11680)) %>% calculate_test(method = "wilcox")
res <- calculate_test(df, method = "wilcox")
res <- select(res, -c("No", "Yes", "fc2", "sd", ".y.", "n1", "n2", "NA", "statistic", starts_with("group"))) %>%
    tibble() %>%
...
get_top <- function(fc_threshold, p_threshold, n = 10) {
    bind_rows(
        res %>%
...
top_genes <- get_top(1.2, 0.01, 1000)
get_top(0.8, 0.01, 1000) %>%
...
ggplot(res, aes(log2fc, log10p)) +
    geom_point(aes(color = Expression), size = 1.5, alpha = 0.5) +
